(function (namespace, $) {
    ...
    function filter_empty_entries(unfiltered_list) {
        var result = [];
        var i;
        var items = unfiltered_list.split(", ");
        for (i = 0; i < items.length; i++) {
    ...
    function load_player_information() {
        $.ajax({
            ...
            success: function (data) {
                if (data.status == 200) {
                    $("#tshockweb_username").html(data.result.output.nickname + "<br><small style='color:white'>" + data.result.output.group + "</small>");
